...
import Header from 'features/Dashboard/components/container/Header';
...
function Home({ children, name }) {
  ...
  useLayoutEffect(() => {
    let unmounted = false;
    if (width >= 1030) !unmounted && dispatch(setOpen(false));
    else !unmounted && dispatch(setOpen(true));
    return () => (unmounted = true);
  }, [width, dispatch]);
  ...
  return (
    <>
      {loading ? (
        ...
        <main
          className={clsx(styles.homePage, {
            [styles.active]: open,
          })}
        >
          <Container fluid>

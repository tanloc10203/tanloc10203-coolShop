  ...
  return (
    <>
      <ListGroupItem action>
        <div
          className="sidebar-main__nav-item"
          id={navArr?.id}
          onClick={() => setStatus(!status)}
        >
          <Link
            to={navArr?.path}
            className="sidebar-main__item"
          >
            <FontAwesomeIcon icon={navArr?.icon_main} />

            <div
              className="sidebar-main__item-link"
              onClick={() => setStatus(!status)}
            >
              {navArr?.label}
            </div>
      ...
      <div>
        <ListGroup className="sidebar-main__submenu">
          ...
          >
            {navArr?.sub_menu
              ? navArr.sub_menu.map((item, index) => (
                <SubItem subMenu={item} key={index} />
              ))
              : null
            }
          </UncontrolledCollapse>
